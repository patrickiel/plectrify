...
/**
    ...
    Two kinds of field on a package look alike and are not: `kind` decides
    whether the payload is executed, while `category` and `tags` decide only
    which heading it appears under and which chips it answers to. Keep display
    code away from the first and install code away from the other two.
*/
...
struct CataloguePackage
{
    ...
    /** Cross-cutting labels the Packages panel offers as filter chips — what
        this package *is*, as against the one place it is filed.

        A second cosmetic field rather than more `category`, because they answer
        different questions and a package has one answer to the first and
        several to the second: a multi-effect rack is a distortion and a delay
        and a reverb, and it sits in exactly one section. The chips were built
        from the top-level heading until that showed its limit — with everything
        on offer filed under "Effects", the row read "Effects 22" and narrowed
        nothing.

        Empty is a legitimate answer, not an oversight: an entry with no tag
        appears under no chip. Cosmetic to exactly the same depth `category` is,
        and read just as leniently — `kind` is still what decides whether any of
        this is executed. */
...
/** Where to get something Plectrify does not host — amp captures, IRs, and in
    ...
    `tags` is the same chip vocabulary a package carries, and the reason links
    have it at all: a chip asks what someone is after ("reverb", "amps"), and
    the honest answer often includes something Plectrify does not host. */
struct CatalogueLink
{
    juce::StringArray category, tags;
