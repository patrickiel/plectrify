...
/**
    The application shell. Owns the audio stack (device manager, processor
    player, rack graph) and hosts the Svelte UI inside a juce::WebBrowserComponent.

    UI <-> engine is an event-only bridge:
      - JS -> C++ via window.__JUCE__.backend.emitEvent(...)  (withEventListener)
      - C++ -> JS via emitEventIfBrowserIsVisible(...)        (addEventListener)
    The web assets are served from ui/dist through JUCE's resource provider, so
    the native __JUCE__ integration is available to the page.
*/
...
private:
    // --- Web UI bridge ----------------------------------------------------
    juce::String uiNavigationTarget() const;
    ...
    void handleStartWindowResize (const juce::var& payload);
    ...
    // CSS cannot reach.
    void handleSetWindowTheme (const juce::var& payload);
    ...
    // re-pushed whenever they do (see changeListenerCallback / scanForPlugins).
    ...
    /** Drops every plugin instance: closes editors, dissolves splits, detaches
        this component as a parameter listener, and removes the nodes. Shared by
    ...
    /** True while it would be unsafe or rude to drop into standby. Each of
        ...
        blocked tick as activity and restarts the idle clock. */
    ...
    std::pair<double, int> currentRateAndBlock() const;
    ...
    void handleSetAudioDevice (const juce::var& payload);
    ...
    void handleWatchInputLevels (const juce::var& payload);
    void emitInputLevels();

    // --- MIDI -------------------------------------------------------------
    // Raw transport only: MidiInputManager opens every input and batches the
    // trigger-capable messages; the UI owns what they mean (rig/scene
    // switching, tuner toggle, MIDI learn) and the binding table.
    ...
    // TONE3000. Destroyed before the web view: on Windows it owns a second
    // WebView2 environment whose COM release is asynchronous.
    std::unique_ptr<Tone3000Service> tone3000;

    // Looper session archive (WAVs under looper-sessions/). Created in the
    // constructor; destroyed after the web view so a completion callback can
    // still find it alive (callbacks themselves are SafePointer-guarded).
