...
namespace
{
    ...
    */
    ...
    std::uint64_t decodeMetronomePattern (const juce::String& text)
    {
    ...
    juce::String encodeMetronomePattern (std::uint64_t pattern, int beats)
    {
        juce::String text;
...
MainComponent::MainComponent()
{
    ...
    if (savedState != nullptr)
    {
        ...
        rack.setMetronomeBeatPattern (decodeMetronomePattern (
   ...
   #if JUCE_WINDOWS
    // The app is installed per-machine under Program Files, which is not a
    // writable WebView2 user-data location for a normal user. Keep the browser
    // profile beside the rest of Plectrify's user data instead. (macOS renders
    // in WKWebView, which has no separate runtime profile to place.)
    const auto webViewDataFolder = appDataDir().getChildFile ("WebView2");
    ...
    const auto onRackEdit = [this] (void (MainComponent::*handler) (const juce::var&))
    {
    ...
    auto options = juce::WebBrowserComponent::Options{}
        ...
        // mitigation there is AmpWebBrowserComponent::paint above.
        .withNativeIntegrationEnabled()
        .withResourceProvider ([this] (const juce::String& path) { return provideResource (path); })
        ...
        .withEventListener ("insertModule",     onRackEdit (&MainComponent::handleInsertModule))
        .withEventListener ("replaceModule",    onRackEdit (&MainComponent::handleReplaceModule))
        .withEventListener ("removeModule",     onRackEdit (&MainComponent::handleRemoveModule))
        .withEventListener ("reorder",          onRackEdit (&MainComponent::handleReorder))
        .withEventListener ("moveModule",       onRackEdit (&MainComponent::handleMoveModule))
        .withEventListener ("swapModules",      onRackEdit (&MainComponent::handleSwapModules))
        .withEventListener ("setBypass",        onRackEdit (&MainComponent::handleSetBypass))
        .withEventListener ("setParam",         onRackEdit (&MainComponent::handleSetParam))
        // Batched variants used by scene switches (one bridge hop per switch).
        .withEventListener ("setParams",        onRackEdit (&MainComponent::handleSetParams))
        .withEventListener ("setBypassStates",  onRackEdit (&MainComponent::handleSetBypassStates))
        .withEventListener ("watchParams",      [this] (juce::var v) { handleWatchParams (v); })
        .withEventListener ("openEditor",       onRackEdit (&MainComponent::handleOpenEditor))
        .withEventListener ("createSplit",      onRackEdit (&MainComponent::handleCreateSplit))
        .withEventListener ("addLane",          onRackEdit (&MainComponent::handleAddLane))
        .withEventListener ("removeLane",       onRackEdit (&MainComponent::handleRemoveLane))
        .withEventListener ("moveLane",         onRackEdit (&MainComponent::handleMoveLane))
        .withEventListener ("setLaneMix",       onRackEdit (&MainComponent::handleSetLaneMix))
        .withEventListener ("setLaneSwitch",    onRackEdit (&MainComponent::handleSetLaneSwitch))
        ...
        .withEventListener ("setStandby",       [this] (juce::var v) { handleSetStandby (v); })
        .withEventListener ("standbyCommand",   [this] (juce::var v) { handleStandbyCommand (v); })
        .withEventListener ("startWindowResize",onActivity ([this] (juce::var v) { handleStartWindowResize (v); }))
        .withEventListener ("setWindowTheme",   [this] (juce::var v) { handleSetWindowTheme (v); })
        ...
        .withEventListener ("openAudioSettings",onActivity ([this] (juce::var)   { showAudioSettings(); }))
        ...
        .withEventListener ("requestAudioDevices", onActivity ([this] (juce::var v)
        {
            emitAudioDevices ((bool) v.getProperty ("rescan", false));
        }))
        .withEventListener ("setAudioDevice",    onActivity ([this] (juce::var v) { handleSetAudioDevice (v); }))
        ...
        .withEventListener ("watchInputLevels",  [this] (juce::var v) { handleWatchInputLevels (v); })
        ...
        .withEventListener ("applyModuleState",  onRackEdit (&MainComponent::handleApplyModuleState))
        ...
        // the session, the network and the download root, and MainComponent
        // only routes. The list is short because browsing is not on this side
        // of the bridge at all — `tone3000Connect` opens TONE3000's own window,
    ...
    webView = std::make_unique<AmpWebBrowserComponent> (options);
    addAndMakeVisible (*webView);

    // Built after the web view, because its first act may be to emit a session
    // state, and destroyed before it — the sign-in window is a second WebView2
    ...
    tone3000 = std::make_unique<Tone3000Service> (Tone3000Service::Delegate {
        ...
        [this] (const juce::String& eventId, const juce::var& payload)
        {
            if (webView != nullptr)
    ...
    midi = std::make_unique<MidiInputManager> (
        [this] (const juce::var& events)
        {
            noteStandbyActivity();
            if (webView != nullptr)
                webView->emitEventIfBrowserIsVisible ("midiEvents", events);
        },
        [this] (const juce::StringArray&) { emitMidiDevices(); });
...
MainComponent::~MainComponent()
{
    ...
    saveAudioDeviceState();       // capture setup while the device is still open
    ...
    editorWindows.clear();
    // Before the web view: on Windows the sign-in window owns a second WebView2
    // environment, and two of them tearing down COM at once is the failure this
    // ordering avoids.
    tone3000.reset();
    webView.reset();
...
void MainComponent::parentHierarchyChanged()
{
    ...
    juce::Timer::callAfterDelay (500, [safe]
    {
        ...
        safe->webView->goToURL (safe->uiNavigationTarget());
        safe->startTimerHz (15); // safe now that goToURL has been called at least once
    });
}

// ---------------------------------------------------------------------------
// Where the WebView first navigates. Normally the resource-provider root, which
// serves the built ui/dist through provideResource(). In Debug, if PLECTRIFY_DEV_URL
// is set (scripts/run.windows.ts -Dev points it at Vite's http://localhost:5173), we load that
// instead: WebView2 still injects window.__JUCE__ on every document, so the real
// native bridge and audio stay live while the UI hot-reloads on every save.
// Release builds ignore the env var and use the installed resource directory.
// ---------------------------------------------------------------------------
juce::String MainComponent::uiNavigationTarget() const
{
   #if JUCE_DEBUG
...
// ---------------------------------------------------------------------------
// Resource provider — serves the installed Svelte app from the staged ui
// directory: next to the executable on Windows, inside the bundle at
// Contents/Resources/ui on macOS (currentExecutableFile there is the binary in
// Contents/MacOS, which carries no resources). Debug builds fall back to the
// source tree's ui/dist so the dev workflow needs no copying; Release binaries
// carry no dev path and rely on the run scripts or the installer staging ui/.
// ---------------------------------------------------------------------------
std::optional<juce::WebBrowserComponent::Resource>
    MainComponent::provideResource (const juce::String& path) const
{
   #if JUCE_MAC
    auto dist = juce::File::getSpecialLocation (juce::File::currentApplicationFile)
                    .getChildFile ("Contents/Resources/ui");
   #else
    auto dist = juce::File::getSpecialLocation (juce::File::currentExecutableFile)
                    .getParentDirectory()
                    .getChildFile ("ui");
   #endif
...
// ---------------------------------------------------------------------------
juce::var MainComponent::buildRackState() const
{
...
juce::var MainComponent::buildRoutingState() const
{
...
void MainComponent::emitRackChanged()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("rackChanged", buildRackState());
}

juce::var MainComponent::buildStatusState()
{
    ...
    status->setProperty ("metronomePattern", encodeMetronomePattern (
    ...
    status->setProperty ("cpuLoad", deviceManager.getCpuUsage());
    ...
    const auto [sampleRate, bufferSize] = currentRateAndBlock();
    ...
    // glitched" into something reproducible. JUCE reports -1 for a driver that
    // cannot count xruns, which the UI shows as unknown rather than as zero.
    status->setProperty ("audioXRuns", deviceManager.getCurrentAudioDevice() != nullptr
                                           ? deviceManager.getCurrentAudioDevice()->getXRunCount()
                                           : -1);
    ...
    if (auto* device = deviceManager.getCurrentAudioDevice())
        totalLatency = device->getInputLatencyInSamples()
...
void MainComponent::emitStatusChanged()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("statusChanged", buildStatusState());
}

juce::var MainComponent::buildPluginsState()
{
...
void MainComponent::emitPluginsChanged()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("pluginsChanged", buildPluginsState());
}

juce::var MainComponent::buildAppInfoState()
{
    ...
    if (auto* device = deviceManager.getCurrentAudioDevice())
    {
        ...
        info->setProperty ("audio", juce::var (audio));
    }
    // No device: the group is absent and the report dashes those rows out,
    // rather than reporting a device that isn't open.
...
void MainComponent::emitAppInfo()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("appInfo", buildAppInfoState());
}

void MainComponent::emitMidiDevices()
{
    if (webView == nullptr || midi == nullptr)
        return;

    auto* state = new juce::DynamicObject();
    juce::Array<juce::var> devices;

    for (const auto& name : midi->openDeviceNames())
        devices.add (name);

    state->setProperty ("devices", juce::var (std::move (devices)));
    webView->emitEventIfBrowserIsVisible ("midiDevicesChanged", juce::var (state));
}
...
void MainComponent::emitPluginScanChanged (const juce::String& status)
{
    if (webView == nullptr)
        return;
    ...
    webView->emitEventIfBrowserIsVisible ("pluginScanChanged", juce::var (state));
}

juce::var MainComponent::buildPluginBlacklistState()
{
...
void MainComponent::emitPluginBlacklist()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("pluginBlacklistChanged", buildPluginBlacklistState());
}

void MainComponent::handleRetryBlacklistedPlugins (const juce::var& payload)
{
...
juce::var MainComponent::buildCatalogueState()
{
    ...
    for (const auto& item : catalogue.getItems())
    {
        ...
        entry->setProperty ("category", toVarArray (package.category));
        entry->setProperty ("tags", toVarArray (package.tags));
    ...
    {
        auto* entry = new juce::DynamicObject();
        entry->setProperty ("category", toVarArray (link.category));
        entry->setProperty ("tags", toVarArray (link.tags));
...
void MainComponent::emitCatalogueState()
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("catalogueState", buildCatalogueState());
}

void MainComponent::handleRequestCatalogue (const juce::var& payload)
{
...
void MainComponent::handleInstallPackages (const juce::var& payload)
{
    ...
    const auto onProgress = [safe, requestId] (CatalogueInstaller::Progress p)
    {
        ...
        if (safe->webView != nullptr)
            safe->webView->emitEventIfBrowserIsVisible ("installProgress", juce::var (event));
    };
    ...
    const auto onFinished = [safe, requestId, bundleId, rescanAll] (CatalogueInstaller::Result result)
    {
        ...
        if (safe->webView != nullptr)
            safe->webView->emitEventIfBrowserIsVisible ("installFinished", juce::var (event));
    ...
    if (! catalogue.installAsync (ids, onProgress, onFinished))
    {
        ...
        if (webView != nullptr)
            webView->emitEventIfBrowserIsVisible ("installFinished", juce::var (event));
        return;
    }
...
void MainComponent::handleCancelInstall (const juce::var&)
{
    catalogue.cancel();
}

void MainComponent::handleUninstallPackages (const juce::var& payload)
{
    juce::StringArray ids;
    if (auto* list = payload.getProperty ("ids", {}).getArray())
        for (const auto& id : *list)
...
void MainComponent::timerCallback()
{
    ...
    if (paramsChanged || stateChanged)
    {
        ...
        webView->emitEventIfBrowserIsVisible ("pluginStateChanged", juce::var (o));
    }
    ...
    // it is max-hold, so leaving it to accumulate would fire a spurious wake
    // the moment the user enables standby.
    ...
    webView->emitEventIfBrowserIsVisible ("paramValues", modules);
}
...
std::optional<juce::AudioProcessorGraph::NodeID>
    MainComponent::findNode (const juce::String& moduleId) const
{
    for (const auto& slot : rack.getSlots())
...
void MainComponent::handleInsertModule (const juce::var& payload)
{
...
void MainComponent::handleReplaceModule (const juce::var& payload)
{
    ...
    const auto [sampleRate, blockSize] = currentRateAndBlock();
    ...
    pluginManager.createInstanceAsync (*desc, sampleRate, blockSize,
        ...
        (std::unique_ptr<juce::AudioPluginInstance> instance, const juce::String& error)
        {
            ...
            if (auto* added = safe->rack.getPluginInstance (nodeID))
                added->addListener (safe.getComponent());
            if (laneId.isNotEmpty())
            ...
            if (auto* outgoing = safe->rack.getPluginInstance (oldNode))
                outgoing->removeListener (safe.getComponent());
...
void MainComponent::handleRemoveModule (const juce::var& payload)
{
...
void MainComponent::handleReorder (const juce::var& payload)
{
...
void MainComponent::handleMoveModule (const juce::var& payload)
{
...
void MainComponent::handleSwapModules (const juce::var& payload)
{
...
void MainComponent::handleSetBypass (const juce::var& payload)
{
    if (auto node = findNode (payload["id"].toString()))
    {
...
void MainComponent::handleSetParam (const juce::var& payload)
{
...
void MainComponent::handleSetParams (const juce::var& payload)
{
...
void MainComponent::handleSetBypassStates (const juce::var& payload)
{
...
void MainComponent::handleWatchParams (const juce::var& payload)
{
...
void MainComponent::emitParamTexts()
{
    ...
    webView->emitEventIfBrowserIsVisible ("paramTexts", modules);
}

void MainComponent::handleOpenEditor (const juce::var& payload)
{
    if (auto node = findNode (payload["id"].toString()))
        openEditorFor (*node);
}

void MainComponent::handleCreateSplit (const juce::var& payload)
{
...
void MainComponent::handleAddLane (const juce::var& payload)
{
...
void MainComponent::handleRemoveLane (const juce::var& payload)
{
    const auto laneId = payload["laneId"].toString();
...
void MainComponent::handleMoveLane (const juce::var& payload)
{
    rack.moveLane (payload["laneId"].toString(), (int) payload["toIndex"]);
    emitRackChanged();
}

void MainComponent::handleSetLaneMix (const juce::var& payload)
{
...
void MainComponent::handleSetLaneSwitch (const juce::var& payload)
{
    rack.setLaneSwitch (payload["groupId"].toString(), payload["activeLaneId"].toString());
    emitRackChanged();
}

void MainComponent::handleSetStatus (const juce::var& payload)
{
    auto* object = payload.getDynamicObject();
    if (object == nullptr)
        return;
...
void MainComponent::handleLooperCommand (const juce::var& payload)
{
...
void MainComponent::handleMetronomeCommand (const juce::var& payload)
{
    const auto action = payload["action"].toString();
    if (action == "toggle")
...
void MainComponent::saveLooperSessionBeforeClear()
{
    ...
    looperSessions->saveAsync (std::move (audio), length, rate,
        ...
        {
            if (safe == nullptr || ! result.ok || safe->webView == nullptr)
                return;
            ...
            safe->webView->emitEventIfBrowserIsVisible ("looperSessionSaved", juce::var (r));
        });
}

void MainComponent::handleLooperLoadSession (const juce::var& payload)
{
...
void MainComponent::emitLooperSessionLoaded (const juce::var& requestId, bool ok)
{
    auto* r = new juce::DynamicObject();
    r->setProperty ("requestId", requestId);
    r->setProperty ("ok", ok);
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("looperSessionLoaded", juce::var (r));
}

void MainComponent::handleRevealAppFolder (const juce::var& payload)
{
    juce::File dir;
...
void MainComponent::handleOpenExternalUrl (const juce::var& payload)
{
...
// ---------------------------------------------------------------------------
juce::Array<juce::var> MainComponent::captureRackEntries() const
{
...
void MainComponent::handleCaptureRig (const juce::var& payload)
{
    ...
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("rigCaptured", juce::var (result));
}

juce::String MainComponent::captureStateOf (const juce::String& moduleId,
...
void MainComponent::handleCaptureModuleState (const juce::var& payload)
{
    ...
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("moduleStateCaptured", juce::var (result));
}

juce::String MainComponent::findNamPluginId()
{
...
void MainComponent::captureFactoryState (const juce::String& pluginId,
...
void MainComponent::handleApplyModuleState (const juce::var& payload)
{
...
void MainComponent::tearDownRack()
{
    editorWindows.clear();
...
void MainComponent::handleApplyRig (const juce::var& payload)
{
...
int MainComponent::beginRigLoadMute()
{
    const int generation = ++rigLoadGeneration;
...
void MainComponent::endRigLoadMute (int generation)
{
...
// --- Auto Standby ----------------------------------------------------------
bool MainComponent::standbyIsBlocked() const
{
...
void MainComponent::noteStandbyActivity()
{
    standby.noteActivity (juce::Time::getMillisecondCounter());
}

bool MainComponent::wakeForRackEdit()
{
    noteStandbyActivity();
...
void MainComponent::abandonStandby()
{
    parkedRack = juce::var();
...
void MainComponent::enterLightStandby()
{
...
void MainComponent::exitLightStandby()
{
...
void MainComponent::enterDeepStandby()
{
...
void MainComponent::beginWakeFromDeepStandby()
{
    if (! isParked())
    {
...
void MainComponent::handleSetStandby (const juce::var& payload)
{
    auto* object = payload.getDynamicObject();
    if (object == nullptr)
        return;
...
void MainComponent::handleStandbyCommand (const juce::var& payload)
{
    const auto action = payload["action"].toString();
    const auto now = juce::Time::getMillisecondCounter();
...
void MainComponent::emitRigApplyProgress (const juce::var& requestId, int current, int total,
                                          const juce::String& pluginName, bool done)
{
    ...
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("rigApplyProgress", juce::var (progress));
}

void MainComponent::applyRigEntries (std::shared_ptr<juce::Array<juce::var>> entries,
                                     ...
                                     std::shared_ptr<std::function<void()>> onFinished)
{
    ...
    if (index >= entries->size())
    {
        ...
        if (! requestId.isVoid())
        {
            ...
            if (webView != nullptr)
                webView->emitEventIfBrowserIsVisible ("rigApplied", juce::var (result));
        }
    ...
    // arrive after this component (and the rack) have been destroyed.
    juce::Component::SafePointer<MainComponent> safe (this);
    pluginManager.createInstanceAsync (desc, sampleRate, blockSize,
...
// ---------------------------------------------------------------------------
juce::File MainComponent::appDataDir() const
{
    return plectrify::appDataDir();
}

bool MainComponent::resolveAppFile (const juce::String& rel, juce::File& out) const
{
...
juce::File MainComponent::sharedPatchesDir()
{
    return CatalogueInstaller::contentDirectory ("patches");
}

#if defined(PLECTRIFY_CONTENT_SOURCE_DIR)
std::map<juce::String, juce::File> MainComponent::sharedPatchSources()
{
...
juce::File MainComponent::sharedPatchSourceDir (const juce::String& patchId)
{
...
juce::StringArray MainComponent::sharedPatchSourceIds()
{
    juce::StringArray ids;
...
bool MainComponent::resolveSharedSourceFile (const juce::String& rel, juce::File& out)
{
...
bool MainComponent::resolveSharedFile (const juce::String& rel, juce::File& out) const
{
...
bool MainComponent::resolveReadableFile (const juce::var& payload, const juce::String& rel,
                                         juce::File& out) const
{
...
bool MainComponent::resolveWritableFile (const juce::var& payload, const juce::String& rel,
                                         juce::File& out) const
{
...
void MainComponent::handleWriteFile (const juce::var& payload)
{
    bool ok = false;
    juce::File f;
    ...
    r->setProperty ("ok", ok);
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("fileWritten", juce::var (r));
}

void MainComponent::handleReadFile (const juce::var& payload)
{
    juce::File f;
    ...
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("fileRead", juce::var (r));
}

void MainComponent::emitFileReadChunk (juce::var requestId, std::shared_ptr<juce::String> text,
                                       juce::String encoding, int offset, int chunkIndex)
{
    ...
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("fileReadChunk", juce::var (chunk));
                                     ...
                                     encoding = std::move (encoding), end, chunkIndex]
    {
        ...
        if (safe->webView != nullptr)
            safe->webView->emitEventIfBrowserIsVisible ("fileReadChunk", juce::var (done));
    });
}

void MainComponent::handleListFiles (const juce::var& payload)
{
    juce::File dir;
   ...
   #endif
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("filesListed", juce::var (r));
}

void MainComponent::handleDeleteFile (const juce::var& payload)
{
    juce::File f;
...
// ---------------------------------------------------------------------------
// Native dialogs (audio settings, plugin editors) + plugin instantiation.
// ---------------------------------------------------------------------------
std::optional<juce::PluginDescription> MainComponent::findKnownPlugin (const juce::String& pluginId)
{
    for (const auto& desc : pluginManager.getKnownPluginList().getTypes())
...
void MainComponent::addPluginFromDescription (const juce::PluginDescription& desc, int index,
...
void MainComponent::scanBundledPluginsIfNeeded()
{
...
void MainComponent::requestFullRescan()
{
...
void MainComponent::scanManagedPlugins()
{
    juce::FileSearchPath path;
...
bool MainComponent::startScan (const juce::FileSearchPath& paths)
{
    juce::Component::SafePointer<MainComponent> safe (this);
    const auto refresh = [safe] (const juce::String& status, bool listChanged)
    {
        ...
        // shutdown, hence the SafePointer.
...
juce::var MainComponent::buildAudioDevicesState (bool rescan)
{
    ...
    for (auto* type : deviceManager.getAvailableDeviceTypes())
    {
        ...
        driver->setProperty ("outputDevices", toVarArray (type->getDeviceNames (false)));
        driver->setProperty ("inputDevices", toVarArray (type->getDeviceNames (true)));
    ...
    state->setProperty ("inputChannel", rack.getInputSourceChannel());
    ...
    state->setProperty ("inputChannels", toVarArray (inputChannels));
...
{
    if (webView != nullptr)
        webView->emitEventIfBrowserIsVisible ("audioDevicesChanged", buildAudioDevicesState (rescan));
}
...
void MainComponent::handleSetAudioDevice (const juce::var& payload)
{
    ...
    if (object->hasProperty ("inputChannel"))
        rack.setInputSourceChannel ((int) payload["inputChannel"]);
...
{
    auto* device = deviceManager.getCurrentAudioDevice();
    if (webView == nullptr || device == nullptr)
        return;
    ...
    webView->emitEventIfBrowserIsVisible ("inputLevels", juce::var (state));
}
...
void MainComponent::openEditorFor (juce::AudioProcessorGraph::NodeID nodeID)
{
    ...
    {
        juce::Component::SafePointer<MainComponent> safeOwner (this);
...
{
    return appDataDir().getChildFile ("audio_settings.xml");
}
...
void MainComponent::saveAudioDeviceState()
{
    if (auto xml = deviceManager.createStateXml())
    {
        ...
        xml->setAttribute ("metronomePattern", encodeMetronomePattern (
            rack.getMetronomeBeatPattern(), rack.getMetronomeBeatsPerBar()));
