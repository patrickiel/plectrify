...
MainComponent::MainComponent()
{
    ...
    if (savedState != nullptr)
    {
        ...
        rack.setFeedbackGuardEnabled (savedState->getBoolAttribute ("feedbackGuardEnabled", true));
...
juce::var MainComponent::buildCatalogueState()
{
    ...
    // is the heading path the panel groups by; `kind` travels for completeness
...
void MainComponent::handleInstallPackages (const juce::var& payload)
{
    ...
    const auto onFinished = [safe, requestId, bundleId] (CatalogueInstaller::Result result)
    {
        if (safe == nullptr)
            return;
        ...
        juce::MessageManager::callAsync ([safe, status, listChanged]
        {
            if (safe != nullptr)
            {
                ...
                if (status == "complete")
                {
                    ...
                    if (std::exchange (safe->pendingManagedRescan, false))
                        safe->scanManagedPlugins();
                }
            }
        });
