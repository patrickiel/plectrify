...
/**
    ...
    Their catalogue is a single-page app, so most of that movement never
    surfaces as a navigation the host can see: opening a tone and narrowing the
    list both change the URL from inside the document. The place is therefore
    read out of the page rather than off its navigations (see
    Tone3000BrowserWindow's poll), and these rules are applied to whatever it
    says it is.
...
/** One tone's own page, rather than a list of them. Takes a place as `placeOf`
    produces it.

    Deliberately the one Select page that is never remembered: it is where a
    download is started from, so it is where the window would otherwise reopen
    every time — on the tone the user already has, instead of on the list they
    were reading when they picked it. */
bool isTonePlace (const juce::String& place);
