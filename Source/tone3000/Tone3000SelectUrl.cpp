...
{
    const auto path = pathOf (place);
    const juce::String root { "/api/v1/select/tones" };

    // With the separator, like isSelectPage and for the same reason: a future
    // "/api/v1/select/tonestacks" is a list, not a tone.
    return path == root || path.startsWith (root + "/");
}
