...
private:
    ...
    /** Start a fresh flow without taking the window away: what a picked tone
        does, so the user can carry on browsing and pick another. */
    void renewFlowKeepingWindow();
    void restartFlow();
    /** Stop believing the page until the one being remembered is back on screen
        — see `chasingPlace`. Every fresh flow starts with this. */
    void beginChase();
    /** Replace the view a fresh flow needs, on the next turn of the message
        loop. Shared by both flow restarts, and deferred for both: each is
        reached from inside a callback of the window it is about to replace. */
    void rebuildFlowSoon();
    /** Build the browser window on a fresh authorize URL, replacing whatever is
        there. The one way a flow is ever started: a view that has already
        carried one answers a second authorize navigation with TONE3000's bare
        "Error code: 9" page. Not to be called from inside the window's own
        callbacks — see the implementation. */
    void openBrowserWindow();
    void closeBrowserWindow();
    /** Remember where the user is, if it is somewhere worth reopening on.
        Answers whether that moved, so a caller can persist only a change.

        Fed from the window's poll as well as its navigations, because their
        catalogue changes route without navigating; a tone's own page is
        deliberately never remembered — see the implementation. */
    bool notePlace (const juce::String& url);
    ...
    /** And what it was showing: a Select-flow *list* page with the single-use
        `authorization_id` stripped out (see Tone3000SelectUrl) — read from the
        document once a second, since their catalogue filters and opens tones
        without ever navigating. Kept beside the bounds in plain state.json
        because that is all it is — a path and some filters, no credential and
        no session. The next opening starts a fresh flow as always and
        re-attaches this to the authorization that flow hands out, which is why
        it can be restored at all: saving the URL whole landed on their "session
        expired" page every time, because picking a tone spends the
        authorization in it. */
    juce::String windowPlace;
    /** How far down that page the reader was. Restored with the place and never
        without it: an offset into a different page is a number with no meaning. */
    int windowScroll = 0;
    /** One restore per opening. The restore is itself a navigation, and acting
        on its arrival would loop. */
    bool placeRestored = false;
    /** Getting back to the place above, and not there yet: the flow's own
        landing page, the restore navigation in the air, or the blank frame in
        between.

        Everything on screen until it clears is a page nobody chose, so nothing
        it says is written down — neither where it is (it is where the user is
        *from*) nor how far down it they are (nowhere; it has just loaded). Both
        halves of that were bugs: the landing page overwrote the remembered
        place, and its honest "0" overwrote the offset being restored. Cleared
        by arriving, and by every way of deciding not to go anywhere — a chase
        nothing can end silences those reports for the life of the window. */
    bool chasingPlace = false;
    ...
    /** Hidden rather than destroyed once it has been opened.
        ...
        cannot serve a second pick. That is what `flowSpent` records. A pick the
        user completes answers it immediately, without the window ever going
        away (`renewFlowKeepingWindow`); a flow that ends any other way — a
        cancel, a decline — leaves the flag set for the next showing to
        re-authorise, instead of handing the user a window that will fail the
        moment they choose something. */
    ...
    /** Held by every deferred message this service posts to itself, and dropped
        in the destructor: a rebuild queued from inside the window's page-load
        callback runs on the next message-loop turn, and the app may be shutting
        down by then. */
