...
export interface CataloguePackage {
  ...
  /** Cross-cutting labels the panel offers as filter chips: what this package
   *  is, as against the one place it is filed. A package sits in one section
   *  and is several things — a multi-effect rack is a distortion and a delay
   *  and a reverb — which is why this is a second field rather than more
   *  `category`. Cosmetic to exactly the same depth, and refused the same
   *  kind-naming words. Omit it and the package answers to no chip. */
  tags?: string[];
  name: string;
  purpose: string;
  version: string;
...
export interface CatalogueLink {
  category?: string | string[];
  /** The same chip labels a package carries. Links have them because a chip
   *  asks what someone is after, and the honest answer to "amps" or "captures"
   *  is largely things Plectrify does not host. */
  tags?: string[];
  label: string;
  url: string;
  note?: string;
...
/**
 * Checks an entry's chip tags, for packages and links alike.
 *
 * Stricter than `validateCategory` in one place and looser in another, and both
 * differences are the point. Looser: an array is the only form, because a tag
 * list of one is still a list and the string shorthand only exists on
 * `category` for the catalogues written before paths did. Stricter: a duplicate
 * is refused rather than deduplicated, since a tag repeated in one entry is
 * always a copy-paste slip and never a shorthand for anything.
 *
 * The same refusal of 'Plugins' and 'Content' as headings, for the same reason:
 * these are the words `kind` uses, and a display field that borrows them
 * invites the two to be read as one.
 */
function validateTags(where: string, tags: string[] | undefined): void {
  if (tags === undefined) return;

  if (!Array.isArray(tags)) fail(`${where} 'tags' must be an array of labels.`);

  // An empty array is a package that answers to no chip written as though it
  // had been given one. Omitting the field says the same thing and reads as
  // deliberate.
  if (tags.length === 0) {
    fail(`${where} has an empty 'tags' array; omit the field to leave it untagged.`);
  }
  ...
  for (const tag of tags) {
    if (typeof tag !== 'string' || !tag.trim()) {
      fail(`${where} has an empty tag; omit the field to leave it untagged.`);
    }

    if (/^(plugins?|content)$/i.test(tag.trim())) {
      fail(
        `${where} has tag '${tag}', which names its kind rather than what it is for. Tags are the panel's filter chips — use what a guitarist is after, e.g. 'Delay', 'Reverb', 'Amps'.`,
      );
    }

    if (seen.has(tag.trim())) fail(`${where} repeats the tag '${tag}'.`);
    seen.add(tag.trim());
  }
}
