...
import { byName, mergePatches, storedFromModule, toPatch, type StoredPatch } from './patches';
...
import { NAM_PLUGIN_NAME, namDefaultKnobs, patchesMissingCaptures } from './tone3000';
...
const MOCK_BUNDLE: CatalogueBundle = {
  id: 'starter',
  name: 'Starter bundle',
  description: 'A complete guitar rig: amp, cabinet, and the usual pedals.',
  version: '1',
  packageIds: [
    'neural-amp-modeler',
    'amalgam-jtm45',
...
export class MockEngine implements EngineBridge {
  ...
  async savePatch(moduleId: string, name: string): Promise<string | null> {
    ...
    if (!mod) return null;
    const id = uid('patch');
    this.writePatch(id, { ...storedFromModule(mod, name), state: mockStateOf(mod) });
    return id;
  }
    ...
    this.rack = this.rack.map((m) =>
      m.id !== moduleId
        ...
        : {
            ...
            // neither leaves the module's own name and colour alone.
  ...
  private installMockTone(tone: Tone3000Tone, model: Tone3000Model, moduleId?: string): void {
    ...
    const tick = () => {
      ...
      setTimeout(() => {
        ...
        this.writePatch(patchId, {
          ...base,
          knobs: base.knobs.length > 0 ? base.knobs : namDefaultKnobs(),
  ...
  installPackages(requested: string[], bundleId?: string): void {
    ...
    window.setTimeout(() => {
      ...
      // A real install rescans, which is what makes the plugins appear.
      if (result.installed.length > 0) this.scanPlugins();
    }, delay + 260);
  }
