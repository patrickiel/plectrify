...
export interface EngineBridge {
  ...
  /** Replace a module's knob mapping with a saved patch's, and restore the
      ...
      only. Also *commits* a preview of that module (see `previewPatch`): the
      patch the pointer was resting on is the patch being chosen. */
  loadPatch(moduleId: string, patchId: string): void;
  /** Show a patch on a live module without committing to it: its knob mapping,
      card look and provenance go on, so the module renders as the patch would
      make it — but the plugin's own tone is **not** applied, so nothing about
      the sound changes and the preview costs nothing to undo. The module's
      metadata is snapshotted on the first preview of a run and put back by
      `cancelPatchPreview`; previewing a second patch keeps that first snapshot,
      so a walk down the menu still returns to where it started. Previewing a
      different module ends the previous module's run first. */
  previewPatch(moduleId: string, patchId: string): void;
  /** End a preview run without choosing anything, putting back the mapping and
      look the module had before it. A no-op when this module is not being
      previewed — including after `loadPatch` committed a run, so a menu closing
      behind a choice cannot undo it. */
  ...
  /** Download and install the named plugins, in order. Ids not in the
      ...
      Pass `rescanAll` to end the run with a scan of the whole plugin search
      path instead of the managed directory alone. Only the first-run starter
      install does: that machine has never been scanned, so the plugins the
      user already had in their own folders are found in the same step —
      everywhere else the narrow scan is the point, since the installer can
      only have written to one folder. */
  installPackages(ids: string[], bundleId?: string, rescanAll?: boolean): void;
  /** Subscribe to whether the engine's own first-run starter-bundle install is
      running. The engine decides and starts that run itself — it is the one
      install nobody clicked — so this exists purely so the empty rack can say
      what those first minutes are about instead of looking broken. Replays the
      current value; false for every launch after the first. */
  ...
  // --- Audio device --------------------------------------------------------
  // The setup wizard's half of the engine. Everything the native
  // AudioDeviceSelectorComponent below would ask for, asked in the app's own
  // language and in an order a guitarist can answer: which interface, which
  // jack, how much latency. The engine still owns every decision that needs a
  // device open — the page names things, it never opens them.

  /** Subscribe to the audio device choice: the driver families, what is open,
      and what that device offers. Replays the last known state; empty until the
      first push. Returns an unsubscribe function. */
  subscribeAudioDevices(listener: (state: AudioDevicesState) => void): () => void;
  /** Ask the engine to push the device state again. `rescan` re-enumerates the
      driver families, which on Windows loads every installed ASIO driver in
      turn — slow, and occasionally a vendor dialog — so it is what a Refresh
      button sends and never what an opening panel does. */
  refreshAudioDevices(rescan?: boolean): void;
  /** Change any part of the open device. Absent fields are left alone (see
      `AudioDeviceChange`). No result: the engine re-pushes what the driver
      actually settled on, which is not always what was asked for — a rate or a
      block size the hardware refuses comes back as the nearest one it took. */
  setAudioDevice(change: AudioDeviceChange): void;
  /** Arm per-channel input metering. The one thing that makes "which jack is
      your guitar in?" answerable by playing rather than by guessing: while
      armed, the engine taps every enabled input channel of the device and
      streams their peaks. Costly enough to be deliberate — hold it only while
      something is showing the meters. */
  watchInputLevels(watching: boolean): void;
  /** Subscribe to those peaks, one linear value per enabled input channel,
      indexed as `AudioDevicesState.inputChannels` is. A stream, not state:
      nothing is replayed, and nothing arrives unless `watchInputLevels` is
      armed. */
  subscribeInputLevels(listener: (peaks: number[]) => void): () => void;

  /** Open the native audio-device settings dialog (native engine only). The
      wizard above covers what a player has to answer; this is still the way to
      everything else — channel masks, exclusive modes, MIDI devices. */
  ...
  /** Resolves once the stored preferences have been read — or once it is
      settled that there are none.

      `subscribeAppSettings` replays the *defaults* to a subscriber before the
      file has come back, which is right for everything that renders (a theme, a
      zoom) and wrong for the handful of fields whose default means "this has
      never happened here". `setupCompleted` is one: acting on its default would
      walk an existing user through first-run setup for the half-second before
      their settings land. Await this first. */
