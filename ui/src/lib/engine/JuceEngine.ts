...
import { DEFAULT_APP_INFO, DEFAULT_STATUS_STATE } from './types';
...
export class JuceEngine implements EngineBridge {
  ...
  async savePatch(moduleId: string, name: string): Promise<string | null> {
    ...
    const doc: StoredPatch = { ...storedFromModule(mod, name), ...tone };
    this.patches = [...this.patches, toPatch(id, doc)].sort(byName);
    this.writePatch(path, doc);
    return id;
  }
  ...
  loadPatch(moduleId: string, patchId: string): void {
    ...
    // is rather than blanking a name and colour the user chose.
    ...
    // Mapping first, tone second, and in that order on the wire: this reaches
    // emitWatch(), so the first poll after the state lands already covers the
    // patch's parameters. Registered the other way round, the new knobs would
    // render a tick stale.
    this.afterMetaChange(moduleId, true);
    void this.applyPatchTone(moduleId, patch);
  }
