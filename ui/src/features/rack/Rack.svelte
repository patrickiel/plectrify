<script lang="ts">
  ...
  import { MIN_DRAWER_HEIGHT } from '../../lib/engine/appSettings';
  ...
  interface Props {
    ...
    rigs: Rig[];
    plugins: PluginInfo[];
    /** The plugin catalogue (App-owned), for the module drawer: patches file
        under their package's category heading. */
      ...
      <div class="flex min-h-0 min-w-0 flex-1 flex-col" bind:clientHeight={workspaceColHeight}>
        ...
        <div class="relative flex min-h-0 min-w-0 flex-1 overflow-hidden">
          <div
            ...
            class={[
              'rack-viewport isolate flex min-h-0 min-w-0 flex-1 overflow-auto px-6 pt-8 pb-20 lg:px-10',
          ...
          >
            ...
            {#snippet moduleCard(m: RackModule)}
              <ModuleCard
                ...
                patches={patches.filter((a) => a.pluginName === m.name)}
              ...
              >
                ...
                {#if rackIsEmpty}
                  {#if editMode && plugins.length > 0}
                    ...
                    <!-- The opaque panel keeps the route visually behind the
                       empty state. The circular marker repeats the insert-gap
                       language used everywhere else in the rack. -->
                    <div
                      class="empty-drop-zone relative z-2 mx-auto max-w-[27rem] min-w-[22rem] flex-[1_1_24rem]"
                    ...
                    >
                      <!-- Unchanged under a drag but for the ring lighting up
                           (see .empty-drop-zone.drop-zone-over): the panel says
                           what this place is for, and a hovered gap elsewhere
                           in the chain does not restate what is arriving
                           either — that is the drag image's job. -->
                      <div class="empty-drop-content">
                        <span class="empty-drop-plus" aria-hidden="true">
                          <PlusIcon size={15} weight="bold" />
                        </span>
                        <span class="min-w-0 text-left">
                          <span class="block text-[.8rem] font-semibold text-ink">
                            Add your first module
                          </span>
                          <span class="mt-0.5 block text-[.7rem] leading-[1.45] text-muted">
                            Drag a patch or plugin here to start your signal chain.
                          </span>
                        </span>
                      </div>
                    </div>
                      ...
                      <div class="flex flex-col items-center gap-2">
                        {#if plugins.length === 0}
                          <!-- With nothing scanned yet the drawer has nothing to
                           offer, so the way in is the scan itself. -->
                          ...
                          >
                        {:else}
                          <!-- Modules are added by dragging from the edit-mode
                           drawer, so the empty rack's one action is opening it. -->
                          <Button
                            size="sm"
                          ...
                          <span
                            class="text-[.65rem] text-[color-mix(in_srgb,var(--color-muted)_78%,transparent)]"
                            >{plugins.length} VST3 {plugins.length === 1 ? 'plugin' : 'plugins'} found</span
                          >
                        {/if}
                        ...
                        {#if plugins.length === 0 && !appSettings.catalogueHintDismissed}
...
<style>
  ...
  /* The empty rack is a quiet panel in the chain, not a label laid over it.
     Its opaque surface masks the trunk so the route reads as entering and
     leaving the card. The circular marker carries the same dashed-accent
     language as every insert gap without outlining the whole panel in teal. */
  .empty-drop-zone {
    display: grid;
    place-items: center;
    width: 100%;
    ...
    border: 1px solid color-mix(in srgb, var(--color-ink) 12%, var(--color-panel-solid));
    background: color-mix(in srgb, var(--color-accent) 4%, var(--color-panel-solid));
    color: var(--color-muted);
    box-shadow: inset 0 1px 0 color-mix(in srgb, var(--color-ink) 4%, transparent);
    transition:
      border-color 0.15s ease,
      background 0.15s ease,
      box-shadow 0.15s ease;
  ...
  .empty-drop-plus {
    ...
    border-radius: 999px;
    background: color-mix(in srgb, var(--color-accent) 9%, var(--color-panel-solid));
    color: color-mix(in srgb, var(--color-accent) 82%, var(--color-muted));
    transition:
      border-color 0.15s ease,
      background 0.15s ease,
      color 0.15s ease,
  ...
  .empty-drop-zone.drop-zone-over {
    border-color: var(--color-accent);
    background: color-mix(in srgb, var(--color-accent) 12%, var(--color-panel-solid));
    box-shadow: 0 0 12px color-mix(in srgb, var(--color-accent) 25%, transparent);
  }
  .empty-drop-zone.drop-zone-over .empty-drop-plus {
    border-style: solid;
    border-color: var(--color-accent);
    background: color-mix(in srgb, var(--color-accent) 22%, var(--color-panel-solid));
    color: var(--color-accent);
    box-shadow: 0 0 10px color-mix(in srgb, var(--color-accent) 22%, transparent);
  }
