<script lang="ts">
  ...
  const draggable = $derived(pluginId !== undefined && !renaming && !editingCategory);
  const title = $derived(patch.displayName ?? patch.name);
  ...
  function startDrag(e: DragEvent) {
    if (!draggable || pluginId === undefined || !e.dataTransfer) return;
...
<div
  ...
  ondragend={onDragEnd}
  role="listitem"
  {@attach tooltip(pluginId === undefined ? `${patch.pluginName} is not installed` : undefined)}
>
  ...
  <div class="flex min-h-9 flex-1 flex-col px-2 pt-1.5 pb-2">
    {#if editingCategory}
      <InlineRenameInput
        ...
        onCancel={() => (editingCategory = false)}
      />
    {:else if artwork}
