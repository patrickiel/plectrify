<script lang="ts">
  ...
  import type { Patch, RackModule } from '../../lib/engine/types';
  ...
  interface Props {
    ...
    /** Saved patches available for this module's plugin. */
    patches: Patch[];
    ...
    /** Present only on modules hosting Neural Amp Modeler — the one plugin a
        TONE3000 tone loads into. Absent leaves the patch menu exactly as it
        was, so no other module grows a TONE3000 row. */
    ...
    onRemove: () => void;
    /** Open this tone's page on TONE3000. Present only on a sealed module, and
        absent if the tone carries no URL — an older patch may not. */
    onOpenTone?: () => void;
  ...
  /** A module playing a TONE3000 tone is **sealed**: its knob mapping is fixed
      and the plugin's own editor is not offered.

      Not a restriction for its own sake. Every tone from TONE3000 loads into
      the same plugin with the same six controls in the same cells, and the
      mapping arrives with the tone — so there is nothing here for the mapping
      editor to express, and every affordance it shows is an invitation to
      break a card that was complete when it arrived. The plugin's editor is
      out for the same reason plus one more: it is where a capture is loaded by
      hand, and a module whose tone was chosen on TONE3000 should not also be
      pointing somewhere else. Loading an ordinary patch over it clears the
      provenance and gives the whole editor back. */
  const sealed = $derived(module.tone3000 !== undefined);
  ...
  /** One square, the same on every TONE3000 card.
      ...
      152px is the two-row knob block a TONE3000 module always has — six
      controls, the mapping arrives with the tone. The measurement is kept only
      as a ceiling, so a card that somehow has less room crops rather than
      overflowing. */
  ...
  /** Whether this card is allowed to offer the control editor at all. Sealing
      is the tone's veto (see `sealed`); a missing plugin has no parameters to
      map, so the "+" cells and the pickers would all be empty. */
  const canEditKnobs = $derived(editing && !sealed && !module.missing);
      ...
      >
        ...
        {#if module.tone3000}
          <!-- The T3K mark leads the header, which is where a mark belongs: it
               ...
               picture, not the service it came from. -->
          <button
            type="button"
              ...
              <div class="menu-rows">
                {#if !module.tone3000}
                  <!-- The plugin's own editor. Labelled with the plugin's
                     ...
                     loses nothing to a prefix. Sealed cards have no row at
                     all, for the reason `sealed` gives: the editor is where
                     a capture would be re-pointed by hand. -->
                  <button
                    class="menu-row"
                ...
                {#if canEditKnobs}
                  <!-- The control editor's switch. Near the top because it is
                     ...
                     cannot offer it (a TONE3000 tone, a missing plugin),
                     the same way the affordances themselves are simply not
                     there. -->
                  <button
                    class="menu-row"
