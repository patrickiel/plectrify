  ...
  >
    <div class="flex max-w-[22rem] flex-col items-center gap-[.6rem] p-8 text-center text-ink/70">
      ...
      <p class="text-[.8rem] leading-normal">
        {#if parked}
          Idle for {idleLabel} — plugins unloaded to free memory. Waking reloads the rig, which takes
          a moment.
        {:else}
          Idle for {idleLabel}. Play, click, or press a key to pick up where you left off.
        {/if}
      </p>
