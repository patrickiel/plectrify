...
<style>
  ...
  .tuner-scale {
    ...
    /* Ticks at the centre and the ±25c marks only — none at either edge, so the
       ...
       Every band straddles its position — pos ± stroke/2 — the same way the
       centre line below does, and the same way the needle and the ±markers
       centre themselves with translateX(-50%). Running a band from pos - stroke
       to pos instead put the whole stroke on one side of the mark, which is
       half a stroke of drift between the centre tick and the needle sitting on
       it: invisible at the footer's 1px, plainly off at the stage tuner's 3. */
    background-image:
      linear-gradient(
        to right,
        transparent calc(var(--tick-lo) - var(--scale-stroke) / 2),
        var(--scale-line) calc(var(--tick-lo) - var(--scale-stroke) / 2)
          calc(var(--tick-lo) + var(--scale-stroke) / 2),
        transparent calc(var(--tick-lo) + var(--scale-stroke) / 2)
          calc(50% - var(--scale-stroke) / 2),
        var(--scale-line) calc(50% - var(--scale-stroke) / 2) calc(50% + var(--scale-stroke) / 2),
        transparent calc(50% + var(--scale-stroke) / 2)
          calc(var(--tick-hi) - var(--scale-stroke) / 2),
        var(--scale-line) calc(var(--tick-hi) - var(--scale-stroke) / 2)
          calc(var(--tick-hi) + var(--scale-stroke) / 2),
        transparent calc(var(--tick-hi) + var(--scale-stroke) / 2)
      ),
      linear-gradient(
        to bottom,
        transparent calc(50% - var(--scale-stroke) / 2),
