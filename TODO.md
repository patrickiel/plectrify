...
- [ ] Feedback-guard slideout copy: note it is tuned for a live guitar input —
      ...
      off either way).
- [x] AU editor windows could not be user-resized anywhere (not just
      GarageBand — Live's AU too; AUv2 has no way to declare a view
      resizable, so no host offers frame dragging). Fixed with the page's own
      corner grip in plugin mode: `EditorResizeGrip` → `setEditorSize` →
      `HostServices::handleSetEditorSize` → `setSize`, the plugin-initiated
      resize every host honours. VST3 host-frame resizing unchanged.
