...
- `InputRouterProcessor` — fixed first node; copies the chosen hardware input
  channel onto both channels (a guitar is mono) via an `std::atomic` source
  index set from the UI thread.
- `OutputLevelProcessor` (same header) — fixed final node: master gain, meter,
  ...
  **Rising is free, and that is half the speed.** An earlier version asked for a
  ...
  `audio_settings.xml`; the latch deliberately is not.
...
- `audio_settings.xml` — last audio device state + input/output gain and tuner
  state (`MainComponent`).
...
authorization — in the same window, and then back to where the user was.
...
`restartFlow`, once per opening so an expiry loop cannot reload the window under
...
number, and this window has no native bridge, so nothing can come the other
way), and re-applied on arrival until the lazily-loaded grid is long enough to
accept it.
...
editing and no plugin editor, because every tone here is the same plugin with
the same six controls in the same cells, the mapping arrives with the tone, and
the plugin's editor is where a capture would be re-pointed by hand. Module-level
editing — move, rename, colour, bypass, patches — is untouched. A TONE3000
patch's drawer tile
