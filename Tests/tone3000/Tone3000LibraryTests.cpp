...
void testSelectUrls()
{
    ...
    // The one Select page that is never remembered: it is where every download
    // is started from, so remembering it would reopen the window on the tone the
    // user already has instead of on the list they picked it from.
    check (isTonePlace (placeOf (tone, base)), "a tone's own page is recognised as one");
    check (! isTonePlace (placeOf (filtered, base)), "a filtered list is not");
    check (! isTonePlace ("/api/v1/select"), "nor is the catalogue itself");
    check (! isTonePlace ("/api/v1/select/tonestacks"),
           "matched with the separator, so a longer path that merely starts the same way is "
           "still a list");
    check (isTonePlace ("/api/v1/select/tones/02-vox-ac306-70408?ir=1"),
           "a query on a tone's page does not make it a list");
